...
//Blinn-Phong Model：与上者区别在于对镜面反射的处理，这里镜面反射光的强度不再取决于反射光与视角之间角度的cos，而取决于入射光与视角的角平分线与法线之间角度的cos
...
vec3 CalcLightPoint(LightPoint light, vec3 uNormal, vec3 dirToCamera) {
	...
	float specIntensity = CalcSpecIntensity(light.pos, lightDir, uNormal, dirToCamera);
	//float specIntensity = CalcSpecIntensity(light.pos, uNormal, dirToCamera) * attenuation;
	vec3 specColor = specIntensity * light.color * texture(material.specular, TexCoord).rgb;
...
vec3 CalcLightSpot(LightSpot light, vec3 uNormal, vec3 dirToCamera) {
	...
	// specular		pow(max(dot(R, Cam), 0), shininess)
	vec3 lightDir = normalize(light.pos - FragPos);
	float specIntensity = CalcSpecIntensity(light.pos, lightDir, uNormal, dirToCamera) * attenuation * spotRatio;
	vec3 specColor = specIntensity * light.color * texture(material.specular, TexCoord).rgb;
